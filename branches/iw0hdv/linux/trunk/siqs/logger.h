/*
     (C) 2012 Claudio Scordino
     No license on original code, see  http://retis.sssup.it/~scordino/code/logger.html
...
#define XDEBUG(p, code) { \
    if (Logger::getInstance().enabled(p)) { \
        code ;\
    } \
}
